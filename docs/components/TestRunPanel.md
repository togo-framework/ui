...
Panel to trigger + show a plugin test run.

## Import
```tsx
import { TestRunPanel } from "@togo-framework/ui";
```

## Props
Rich, presentational props (ported from the prism design system). See the **Docs** tab of
the `TestRunPanel` story in Storybook for the full, auto-generated props table + interactive controls.
