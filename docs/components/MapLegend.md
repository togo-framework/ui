...
Legend for a map panel.

## Import
```tsx
import { MapLegend } from "@togo-framework/ui";
```

## Props
Rich, presentational props (ported from the prism design system). See the **Docs** tab of
the `MapLegend` story in Storybook for the full, auto-generated props table + interactive controls.
