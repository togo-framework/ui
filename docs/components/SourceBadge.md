...
Small badge indicating a plugin's source (official / community / …).

## Import
```tsx
import { SourceBadge } from "@togo-framework/ui";
```

## Props
Rich, presentational props (ported from the prism design system). See the **Docs** tab of
the `SourceBadge` story in Storybook for the full, auto-generated props table + interactive controls.
