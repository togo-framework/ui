...
Map chrome shell (legend + layers panel) around an injected map canvas.

## Import
```tsx
import { MapPanel, MapLegend, MapLayersPanel } from "@togo-framework/ui";
```

## Props
Rich, presentational props (ported from the prism design system). See the **Docs** tab of
the `MapPanel` story in Storybook for the full, auto-generated props table + interactive controls.
