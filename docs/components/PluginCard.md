...
Marketplace plugin card — icon, title, description, source badge, sparkline, select + details actions.

## Import
```tsx
import { PluginCard } from "@togo-framework/ui";
```

## Props
Rich, presentational props (ported from the prism design system). See the **Docs** tab of
the `PluginCard` story in Storybook for the full, auto-generated props table + interactive controls.
