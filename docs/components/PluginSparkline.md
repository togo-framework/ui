...
Compact activity sparkline for a plugin card.

## Import
```tsx
import { PluginSparkline } from "@togo-framework/ui";
```

## Props
Rich, presentational props (ported from the prism design system). See the **Docs** tab of
the `PluginSparkline` story in Storybook for the full, auto-generated props table + interactive controls.
