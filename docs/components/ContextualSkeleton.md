...
Context-aware skeleton placeholder for realtime loading states.

## Import
```tsx
import { ContextualSkeleton } from "@togo-framework/ui";
```

## Props
Rich, presentational props (ported from the prism design system). See the **Docs** tab of
the `ContextualSkeleton` story in Storybook for the full, auto-generated props table + interactive controls.
