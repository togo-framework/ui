...
Two-pane layout shell for a plugin detail page.

## Import
```tsx
import { PluginDetailLayout } from "@togo-framework/ui";
```

## Props
Rich, presentational props (ported from the prism design system). See the **Docs** tab of
the `PluginDetailLayout` story in Storybook for the full, auto-generated props table + interactive controls.
