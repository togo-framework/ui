...
Section-shaped skeleton placeholder.

## Import
```tsx
import { SectionSkeleton } from "@togo-framework/ui";
```

## Props
Rich, presentational props (ported from the prism design system). See the **Docs** tab of
the `SectionSkeleton` story in Storybook for the full, auto-generated props table + interactive controls.
