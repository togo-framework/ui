...
Layer toggle panel for a map.

## Import
```tsx
import { MapLayersPanel } from "@togo-framework/ui";
```

## Props
Rich, presentational props (ported from the prism design system). See the **Docs** tab of
the `MapLayersPanel` story in Storybook for the full, auto-generated props table + interactive controls.
