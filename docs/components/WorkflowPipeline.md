...
Read-only workflow pipeline view — lanes of step cards (the workflow 'manager with steps').

## Import
```tsx
import { WorkflowPipeline } from "@togo-framework/ui";
```

## Props
Rich, presentational props (ported from the prism design system). See the **Docs** tab of
the `WorkflowPipeline` story in Storybook for the full, auto-generated props table + interactive controls.
