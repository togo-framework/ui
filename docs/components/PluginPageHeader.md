...
Header for a plugins listing page (title, count, search/actions slots).

## Import
```tsx
import { PluginPageHeader } from "@togo-framework/ui";
```

## Props
Rich, presentational props (ported from the prism design system). See the **Docs** tab of
the `PluginPageHeader` story in Storybook for the full, auto-generated props table + interactive controls.
