...
Appearance/branding editor section for a plugin (name, icon, colors).

## Import
```tsx
import { PluginAppearanceSection } from "@togo-framework/ui";
```

## Props
Rich, presentational props (ported from the prism design system). See the **Docs** tab of
the `PluginAppearanceSection` story in Storybook for the full, auto-generated props table + interactive controls.
