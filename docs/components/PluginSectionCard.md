...
A titled section card used on plugin pages.

## Import
```tsx
import { PluginSectionCard } from "@togo-framework/ui";
```

## Props
Rich, presentational props (ported from the prism design system). See the **Docs** tab of
the `PluginSectionCard` story in Storybook for the full, auto-generated props table + interactive controls.
